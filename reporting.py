...
def current_conditions_block(pkg):
    curr = pkg["current"]
    lines = [f"<b>Current Conditions ({curr['observed_at']})</b>"] if curr else ["<b>Current Conditions</b>"]
    lines.append(f"<b>{pkg['name']}</b>:")
    if curr:
        ...
        temp_line = f"Temp: {temp_f}°F / {temp_c}°C" if temp_available else "Temp: Not available"
        ...
        if gust_str:
            wind_parts.append(f"gusting to {gust_str}")
...
def save_whatsapp_summary(pkg_dhi, pkg_xc, txt_path, stamp_human=None):
    def section(pkg):
        ...
        wa = []
        wa.append(f"*{name}*")
        wa.append(f"*Current Conditions - {now_et.day} {now_et.strftime('%b @ %I:%M %p %Z')}*")
        if curr:
            temp_str = f"{curr['temp_f']}°F / {curr['temp_c']}°C" if curr['temp_c'] is not None else "Not available"
...
def save_whatsapp_combined(pkg_primary, pkg_secondary, txt_path, stamp_human=None):
    ...
    lines.append('')
    lines.append(f'*Current Conditions - {now_et.day} {now_et.strftime("%b @ %I:%M %p %Z")}*')
    for pkg in packages:
        name = pkg['name']
        curr = pkg['current']
        lines.append(f"- *{name}*:")
        if curr:
            temp_str = 'Not available' if curr['temp_c'] is None or curr['temp_f'] is None else f"{curr['temp_f']}°F / {curr['temp_c']}°C"
...
def save_markdown(pkg_dhi, pkg_xc, md_path):
    def section(pkg):
        ...
        md = []
        md.append(f"## {name}\n")
        md.append(f"### Current Conditions - {now_et.day} {now_et.strftime('%b @ %I:%M %p %Z')}*")
        if curr:
            md.append(f"**Observed at {curr['observed_at']}**")
            wind = f"{curr['wind_mph']} mph" if curr['wind_mph'] is not None else "—"
            gust = f", gusting to {curr['gust_mph']} mph" if curr['gust_mph'] else ""
            wdir = f" from {deg_to_cardinal(curr['wind_dir_deg'])}" if curr['wind_dir_deg'] is not None else ""
            md.extend([
                f"- Conditions: {curr['conditions']}",
                f"- Temp: {curr['temp_f']}°F / {curr['temp_c']}°C",
                f"- Wind: {wind}{wdir}{gust}",
                f"- RH: {curr['rh']}%",
...
def build_pdf_full(pkg_dhi, pkg_xc, pdf_path):
    ...
    for pkg in packages:
        elements.append(Paragraph(current_conditions_block(pkg), styles["SubHeading"]))
        elements.append(Spacer(1, 6))
    elements.append(Paragraph("<br/><i>Note: Current conditions are based on the latest available data and may not be real-time.</i>", styles["NormalSmall"]))

    elements.append(Paragraph("Short-term Forecast (36 Hours)", styles["SubHeading"]))
    for pkg in packages:
    ...
    for pkg in packages:
        ...
        elements.append(make_table(pkg["short_metric"],
               ["Date/Time (EDT)","Temp (°C)","Feels\nLike (°C)","Wind\n(km/h)","Gusts\n(km/h)","Dir","RH (%)","PoP (%)","Cloud (%)"],
        ...
        elements.append(make_table(pkg["short_imper"],
               ["Date/Time (EDT)","Temp (°F)","Feels\nLike (°F)","Wind\n(mph)","Gusts\n(mph)","Dir","RH (%)","PoP (%)","Cloud (%)"],
               [80,45,45,60,60,35,35,35,40]))
